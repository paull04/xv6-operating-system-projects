...
userinit(void)
{
  ...
  p->main_pid = p->pid;
  p->main_thread = p; // Set the main thread pointer
  p->cnt = 0;
  release(&p->lock);
}
...
growproc(int n)
{
  ...
  for(np = proc; np < &proc[NPROC]; np++) {
    if(np->main_pid == p->pid) {
      acquire(&np->lock);
      np->sz = sz; // Update the size for all threads
      release(&np->lock);
    }
  }
  release(&wait_lock);
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        //printf("Running process %d on CPU %d\n", p->pid, cpuid()); printf("epc: %ld\n", p->trapframe->epc);
        p->state = RUNNING;
        c->proc = p;
